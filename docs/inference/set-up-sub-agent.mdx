...
OpenShell names the sandbox container `openshell-<sandbox>` or `openshell-<sandbox>-<id>`.
Look up the exact name before running admin commands:

```console
$ export SANDBOX=my-assistant
$ export SANDBOX_CTR=$(docker ps --format '{{.Names}}' | grep -E "^openshell-${SANDBOX}(-|$)" | head -1)
...
The config files are owned by `sandbox:sandbox`.
`chmod 644` lets the agent process write the file; `chmod 444` makes it read-only for the agent process, which is sufficient to enforce config integrity at startup because the agent runs as the `sandbox` user.
...
$ cat /tmp/openclaw.updated.json | docker exec -i --user root "$SANDBOX_CTR" sh -c 'cat > /sandbox/.openclaw/openclaw.json'
...
- Refer to [Switch Inference Providers](/inference/switch-inference-providers) to change the primary orchestration model instead of adding a sub-agent model.
