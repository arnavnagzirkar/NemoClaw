...
Resume onboarding or reconnect:
...
1. Resume onboarding (if needed).

   If the sandbox is not listed or the command fails, resume onboarding:
...
Resume onboarding to re-establish gateway trust:
...
Remove the gateway and re-run onboarding, then re-run setup.
