...
After fixing the key, re-enable web search with `nemoclaw <name> config set --key web.backend --value brave`.
...
Show the current shields state for a sandbox.
Reports whether shields are up (config locked, restrictive network policy applied) or down (default mutable state), and whether a timed auto-restore is pending.
...
Raise sandbox security shields.
Locks the sandbox configuration and applies a restrictive network policy so sensitive workloads cannot be reconfigured at runtime.
Restores shields from the saved snapshot if one exists.
...
Temporarily lower sandbox security shields.
Returns the sandbox to the default mutable state.
Use `--timeout` to set a duration after which shields are automatically restored.

```console
$ nemoclaw my-assistant shields down [--timeout <duration>] [--reason <text>] [--policy permissive]
```

| Flag | Description |
|------|-------------|
| `--timeout <duration>` | Duration before shields are automatically restored (for example `5m`, `1h`) |
| `--reason <text>` | Reason for lowering shields (recorded in the audit log) |
| `--policy <name>` | Policy to apply while shields are down (default: `permissive`) |

### `nemoclaw <name> config get`

Read the sanitized agent configuration for a sandbox.
The output redacts secrets so it is safe to share in bug reports.
Use `--key` to read a single dotpath from the config, and `--format` to control output format.
...
| Flag | Description |
|------|-------------|
| `--key <dotpath>` | Dotpath to read from the sanitized config (for example `model`, `web.backend`) |
